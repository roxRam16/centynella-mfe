...
- **Shell (mockup.png):** encabezado con degradado Blue→Violet (`palette.header`), menú lateral negro suave (`palette.sidebar`) con riel de iconos colapsado / extendido que EMPUJA el contenido (en xs se superpone); los remotes solo renderizan su contenido dentro de `<main>` y se agregan solos al menú desde `remoteRegistry` (`layouts/navigation.tsx`).
