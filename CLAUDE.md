...
`src/`: `components` (librería propia de UI sobre MUI), `config`, `context` (sesión), `federation`, `hooks`, `layouts`, `pages`, `routes`, `services`, `theme`, `utils`, `test`. Alias `@/` → `src/`. `mockups/` = diseños de referencia (ignorada por git y Docker; no borrar).
